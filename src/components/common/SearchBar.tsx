...
const SearchBar = () => {
    ...
    const handleInputChange: ChangeEventHandler<HTMLInputElement> = (event) => {
        if (searchInput.current) {
            searchInput.current.value = event.target.value
        }
    }

    return (
        <form onSubmit={handleSubmit} className="bg-zinc-100 py-2 pl-4 pr-9 rounded-md relative focus">
            <label>
                <input type="text"
                       placeholder="Search"
                ...
                <button type="submit" className="absolute right-4 top-1/2 -translate-y-1/2 cursor-pointer">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            ...
                            stroke="#52525B" strokeWidth="1.5" strokeMiterlimit="10"/>
                        <path d="M10.5715 10.5716L14 14" stroke="#52525B" strokeWidth="1.5" strokeMiterlimit="10"
                              strokeLinecap="round"/>
                    </svg>
                </button>
            </label>
